...
export function Formula202020() {
  ...
  return (
    <section ref={container} className="h-screen bg-background overflow-hidden flex flex-col justify-center relative">
      ...
      <div ref={introRef} className="absolute top-10 md:top-20 left-0 w-full text-center z-10 px-4">
      ...
      <div ref={panelsRef} className="flex flex-nowrap w-[300%] h-[60vh] md:h-[70vh] items-center mt-20 md:mt-0 relative z-10">
        {steps.map((step, index) => (
          ...
          >
            <div className="relative group p-8 md:p-16 rounded-3xl bg-card border border-border/50 hover:border-primary/50 transition-colors max-w-2xl w-full mx-auto shadow-2xl backdrop-blur-md">
              ...
              <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-background border-4 border-primary z-20" />

              <div className="text-lg font-mono text-muted-foreground mb-6 text-center">{step.time}</div>
